...
while(not ingreso):
    ...
    if respuesta.lower() == "no":
        ...
        print("Proceso de alta de usuario " +nombre+ " ha sido finalizado. Por favor inicie sesion")
        continue

    elif respuesta.lower() == "si":
            nombre1 = input("Ingrese su nombre:")
            password1 = input("Ingrese su contraseña:")
            if(nombre1 == "Gabriela" and password1 == "qwerty"):
                ingreso=True
            elif (nombre1== nombre and password1==password):
                ingreso=True
            else: print("Contrsaeña incorrecta, intente de nuevo")
...
while(on_line):
    ...
    if opcion==1:
        ...
        for i in lifestore_products:
            ...
            for j in lifestore_sales:
                if j[1] == i[0]:
                    cantidad_ventas += 1
            addition = i[1]
            lista_top5.append([addition, cantidad_ventas, "Vendidos: " + str(cantidad_ventas)])
            temp = i[:]
            temp.append(cantidad_ventas)
            temp.append("Vendidos: " + str(cantidad_ventas))
            lista_productos_ventas.append(temp)
        sorted_list_50 = sorted(lista_top5, key=operator.itemgetter(1), reverse=True)
        print("Los 5 mas vendidos:")
        print(sorted_list_50[0:5])
        ...
        sorted_list_10 = sorted(lista10, key=operator.itemgetter(1), reverse=True)
        print("Los 10 mas buscados")
        print(sorted_list_10[0:10])
        ...
        for i in categorias:
            print("Los 5 menos vendidos, Categoria ", i)
            lista_por_categoria = [item for item in lista_productos_ventas if item[3] == i]
            lista_por_categoria = sorted(lista_por_categoria, key=operator.itemgetter(5), reverse=False)
            if (len(lista_por_categoria) <= 5):
                print("Categoria solo cuenta con " + str(len(lista_por_categoria)) + " elementos")
                print(lista_por_categoria)
            else:
                print(lista_por_categoria[:5])
        ...
        for i in lifestore_products:
            ...
            if cat in categorias:
                ...
                categorias.append(cat)
        for i in categorias:
            print("Los 10 menos Buscados, Categoria ", i)
            lista_por_categoria = [item for item in lista_productos_busquedas if item[3] == i]
            lista_por_categoria = sorted(lista_por_categoria, key=operator.itemgetter(5), reverse=False)
            if (len(lista_por_categoria) <= 10):
                print("Categoria solo cuenta con " + str(len(lista_por_categoria)) + " elementos")
                print(lista_por_categoria)
            else:
                print(lista_por_categoria[:10])
        ...
        sorted_list_5 = sorted(lista_top5_reviews, key=operator.itemgetter(1), reverse=True)
        print("Los 5 mejores reviews:")
        print(sorted_list_5[0:5])
        ...
        sorted_list_5 = sorted(lista_worst5_reviews, key=operator.itemgetter(1), reverse=False)
        print("Los 5 peores reviews:")
        print(sorted_list_5[0:5])

    elif opcion == 7:
        """Reporte de Ganancias"""
        ganancia_meses = [[1, "Enero", 0], [2, "Febrero", 0], [3, "Marzo", 0], [4, "Abril", 0], [5, "Mayo", 0],
                          [6, "Junio", 0], [7, "Julio", 0], [8, "Agosto", 0], [9, "Septiembre", 0], [10, "Octubre", 0],
        ...
        print("Promedio de ventas mensuales: " + str(cantidad_ventas // 12) + "USD")
        sorted_list_meses = sorted(ganancia_meses, key=operator.itemgetter(2), reverse=True)
        print("Meses con mas ventas: ")
        print(sorted_list_meses)
